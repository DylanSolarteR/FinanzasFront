...
function NavButton({text,href,imgsrc}){
    return(
        <Link className='py-4 text-slate-700 hover:bg-slate-800 group rounded transition-colors duration-300 flex' to={href}>
            <img src={imgsrc} className={'mx-1 p-2 rounded-full bg-slate-200'}/>
...
export default function NavSide(){
    ...
    <button className='bg-slate-300 transition-colors duration-300 inline-flex w-full h-10 hover:bg-slate-400 items-center ' onClick={
        ...
        <img src={state==1? "/x.svg":"/menu.svg"} className='fill-slate-100 text-slate-100 px-2 h-full'/>
        <div className={"text-slate-700 text-center mx-auto text-xl"}>
            {state==1? "Menu":""}
        </div>
    </button>
    {navigation.map((dato)=>(<NavButton text={state==1? dato[2]:""} href={dato[0]} imgsrc={dato[1]}/>))}
    </div>
    </>
    )
}